...
"proc-macro" stands for [Procedural Macros](https://doc.rust-lang.org/reference/procedural-macros.html).  There is another kind of macros called [Macros By Example (MBE)](https://doc.rust-lang.org/reference/macros-by-example.html) but IMHO I do not mean to write lexical-analyzer desciplining the coder to follow the syntax I want (actually, using `syn` and `quote` makes it so I don't have to do any of that, just my excuse being lazy), so I will stick to the proc-macro for this demo.
